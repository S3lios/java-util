...
public class JPlot extends JPanel implements MouseInputListener, MouseWheelListener{
	...
	public void activeCursor(boolean cursor) {
		this.cursor = cursor;
		if (cursor) {
			this.setCursor( this.getToolkit().createCustomCursor(
			new BufferedImage( 1, 1, BufferedImage.TYPE_INT_ARGB ),
	...
	public void show() {
		...
		save.addActionListener(e -> {
			...
			if (userSelection == javax.swing.JFileChooser.APPROVE_OPTION) {
				...
				this.savePNG(fileToSave.getAbsolutePath());
			}
		});

		MenuItem autoResize = new MenuItem("Auto resize");
		autoResize.addActionListener(e -> {
			this.minX = Double.POSITIVE_INFINITY;
			this.maxX = Double.NEGATIVE_INFINITY;
			this.minY = Double.POSITIVE_INFINITY;
			this.maxY = Double.NEGATIVE_INFINITY;
			for (Double[] x : this.xData) {
				for (double x2 : x) {
					if (x2 < minX) minX = x2;
					if (x2 > maxX) maxX = x2;
				}
			}
			for (Double[] y : this.yData) {
				for (double y2 : y) {
					if (y2 < minY) minY = y2;
					if (y2 > maxY) maxY = y2;
				}
		...
		fileMenu.add(save);
		imageMenu.add(autoResize);
		frame.setMenuBar(new MenuBar());
		frame.getMenuBar().add(fileMenu);
		frame.getMenuBar().add(imageMenu);
	...
	public void plot(double[] y, int color, String label) {
		double[] x = new double[y.length];
		for (int i = 0; i < y.length; i++) {
			x[i] = i;
		}
		this.plot(x, y, color, label);
	}

	public void plotXY(double[][] xy, int color, String label) {
